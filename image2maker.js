...
function addHeadingText(ctx) {
    ...
    const headingText = 'Heat Fleet Savings';
...
async function getSaveAmounts(locations){
    ...
    const saveAmount = async (hash) => {
      let saving = 0;
      if(hash != "LI"){
        saving = (await reqPrice(1, hash, null, 100, "high", "best", 0) - await reqPrice(1, hash, null, 100, "low", "best", 0))
      }
      else{
        saving = (await reqPrice(2, 1, null, 100, "high", "best", 0) - await reqPrice(2, 1, null, 100, "low", "best", 0))
      }
    ...
    for(let i = 0; i < locations.length; ++i){
      result.push(await saveAmount(locations[i]))
    }
...
async function addContent(ctx){
    ...
    const locations = ["NY", "CT", "MA", "PA", "ME", "LI"]
    const saveAmounts = await getSaveAmounts(locations);
    // const saveAmounts = [ '3.36', '3.07', '1.17', '1.90', '0.91', '2.26' ];
...
async function makeImage2() {
  try {
    const inputImagePath = 'Images/Backgrounds/InstaBackground002CorrectDim.jpg';
    const outputImagePath = 'Images/Result/image_2.jpg';
    ...
    console.log('Image 2 modification completed successfully');
